...
LociiGhost 為個人獨立維護的開源工具，僅以**研究、開發測試、個人地圖瀏覽**等
合法用途為目的提供。使用前請理解以下：

1. **使用風險自負**：本工具改變 iPhone GPS 模擬狀態，可能違反某些 app／遊戲
   的服務條款（特別是 Niantic 旗下 Pokémon GO、Pikmin Bloom 等定位類遊戲），
   導致**帳號封禁、進度遺失、寶物清空**。任何因此產生的損失，作者不負任何
   責任。
2. **僅於開發者本人測試環境驗證**（macOS 15+ Apple Silicon、iPhone 16 Pro Max /
   iOS 18.7–26.4）。不保證其他裝置／iOS／macOS 組合穩定運作。
3. **無支援保證**：本專案為業餘時間維護，無 SLA、無客服。Bug 修復、新 iOS
   相容性、新功能皆視作者時間與精力而定。
4. **遵守當地法律**：請依使用地法律規定操作。本工具不得用於詐欺、騷擾、
   規避地理限制以從事違法行為，或任何造成第三方損害的用途。
5. **依 MIT 散佈、無擔保**：本軟體「依現狀」提供，無任何明示或默示之擔保。
   完整法律條款見 [`LICENSE`](LICENSE)。
