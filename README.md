...
本專案透過 [pymobiledevice3](https://github.com/doronz88/pymobiledevice3)
介接 Apple DVT / RemoteServices 協議，以 LocationSimulation 模擬 GPS
訊號達成虛擬定位。若將本工具用於基於地理位置的遊戲（例如 Pokémon GO、
Ingress、Pikmin Bloom、Monster Hunter Now 等 Niantic 系列）或社交、打卡、
物流類應用，**可能違反該平台的服務條款，進而導致帳號遭警告、限制、封鎖
或永久停權，已累積之虛擬寶物、進度、儲值點數亦可能一併損失**。開發者對
因使用本工具所造成之任何帳號損失、虛擬財產損害或衍生糾紛，**概不負責**。

### 3. 系統與硬體風險

本專案於 WiFi 模式下需以系統管理員權限執行（透過 `osascript` 提權），
以建立 `utun` 虛擬網路介面、與 iOS 裝置協商 RSD（Remote Service Discovery）
通道。雖然程式碼已經內部測試，但開發者不保證於所有 macOS 版本、Apple
Silicon 機型、網路環境下皆能穩定運行。常見的潛在狀況包括：

- 與 VPN 軟體、第三方防火牆或網路虛擬化工具（Little Snitch、Tailscale、
  ProtonVPN 等）發生衝突，導致 Tunnel 建立失敗或系統網路暫時異常。
- 程式非正常結束時殘留的 `utun` 介面需重新啟動 Mac 始能完全清除。
- WiFi 連線中斷時需手動重試或重新啟動 daemon。

使用者應自行評估上述風險並承擔因此所產生之任何後果。本專案僅操作本身
所建立之臨時網路介面與自身設定檔（位於
`~/Library/Application Support/LociiGhost/` 與 `~/Library/Caches/LociiGhost/`），
**不會修改 iOS 裝置內任何使用者資料，亦不會變更 macOS 核心檔案或既有
裝置配對記錄**。
...
Google Geocoding API。地圖顯示之座標、路徑、地址資訊**僅供參考**，開發者
...
本軟體**僅於開發者本人測試環境驗證** —— macOS 15+ Apple Silicon、
...
《著作權法》、《電腦處理個人資料保護法》及相關國際條約。本工具**不得**
...
本軟體依 **MIT License** 散佈，「依現狀」（AS IS）提供，無任何明示或
默示之擔保（包括但不限於適售性、特定目的適用性、無侵權之擔保）。完整
法律條款見 [`LICENSE`](LICENSE)。
